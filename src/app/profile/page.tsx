            ...
            <div className="bg-white rounded-2xl shadow-md p-6 space-y-6 border border-gray-200 mt-4">
                ...
                <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                        Halte <strong>Strg</strong> (Windows) oder <strong>Cmd</strong> (Mac), um mehrere Module
                        auszuwählen.
                    </p>
                </div>
                ...
                {/* Availability */}
                <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Verfügbarkeit (Wochentage &
                        Zeiten)</label>
                        ...
                        {weekdays.map(({key, label}) => (
                            <div key={key} className="flex flex-wrap items-center gap-4">
                                <label className="flex items-center gap-2">
                                    <input
                                        type="checkbox"
                                        checked={availability[key]?.active || false}
                                        onChange={() => toggleDay(key)}
                                    />
                                    <span>{label}</span>
                                </label>
                                {availability[key]?.active && (
                                    <>
                                        <input
                                            type="time"
